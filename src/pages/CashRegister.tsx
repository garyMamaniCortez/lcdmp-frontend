...
import { Plus,  QrCode, Banknote, Lock, ArrowUpRight, ArrowDownRight, X } from 'lucide-react';
  ...
  return (
    <MainLayout>
      <div className="space-y-4 sm:space-y-6 animate-fade-in">
              ...
              <DialogContent className="w-[95vw] sm:w-full max-w-lg rounded-lg max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleAddTransaction} className="space-y-4 px-1">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input type="number" placeholder="0" required className="text-sm" />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea placeholder="Descripción de la transacción..." required className="text-sm" rows={3} />
                  </div>
                ...
                <form onSubmit={handleCloseRegister} className="space-y-4 px-1">
                  <div className="p-3 sm:p-4 bg-muted/50 rounded-lg space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">Bs. {mockCashRegister.openingBalance}</span>
                    </div>
                    <div className="flex justify-between text-green-600">
                      <span>+ Ingresos:</span>
                      <span className="font-medium">Bs. {totalSales}</span>
                    </div>
                    <div className="flex justify-between text-red-600">
                      <span>- Gastos:</span>
                      <span className="font-medium">Bs. {totalExpenses}</span>
                    </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input type="number" placeholder="0" defaultValue={cashTotal} required className="text-sm" />
                  </div>

                  <div className="space-y-1.5">
                    <Label className="text-sm">Saldo inicial para mañana (Bs.)</Label>
                    <Input type="number" placeholder="500" defaultValue="500" required className="text-sm" />
                  </div>

                  <div className="space-y-1.5">
                    <Label className="text-sm">Notas (opcional)</Label>
                    <Textarea placeholder="Observaciones del cierre..." className="text-sm" rows={3} />
                  </div>
        ...
        {/* Summary Cards - Mobile: 2 columnas, Tablet/Desktop: 4 columnas */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 px-4 sm:px-0">
          <Card>
            <CardContent className="p-3 sm:p-4">
              ...
              <p className="text-lg sm:text-2xl font-bold mt-2 sm:mt-3">Bs. {cashTotal}</p>
          ...
          <Card>
            <CardContent className="p-3 sm:p-4">
              ...
              <p className="text-lg sm:text-2xl font-bold mt-2 sm:mt-3">Bs. {qrTotal}</p>
          ...
          <Card>
            <CardContent className="p-3 sm:p-4">
              ...
              <p className="text-lg sm:text-2xl font-bold mt-2 sm:mt-3 text-green-600">+Bs. {totalSales}</p>
          ...
          <Card>
            <CardContent className="p-3 sm:p-4">
              ...
              <p className="text-lg sm:text-2xl font-bold mt-2 sm:mt-3 text-red-600">-Bs. {totalExpenses}</p>
        ...
        <Card className="mx-4 sm:mx-0">
          <CardContent className="p-3 sm:p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-base sm:text-xl font-bold">Bs. {mockCashRegister.openingBalance}</p>
            </div>
            <Badge variant={mockCashRegister.status === 'open' ? 'default' : 'secondary'} className="text-xs sm:text-sm">
                  ...
                  <TableBody>
                    {mockCashRegister.transactions.map(transaction => (
                      <TableRow key={transaction.id}>
                        ...
                        <TableCell className={`text-right font-medium whitespace-nowrap ${transaction.amount < 0 ? 'text-red-600' : 'text-green-600'}`}>
                          {transaction.amount > 0 ? '+' : ''}Bs. {transaction.amount}
                        </TableCell>
                      </TableRow>
                    ))}
